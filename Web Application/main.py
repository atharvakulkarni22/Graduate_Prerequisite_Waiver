...
def success():
    try: 
        if request.method == 'POST':   
            ...
            selected_prerequisite = request.form.get('selectedPrerequisite')
            ...
            if(selected_prerequisite == "6"):
                sim = get_word2vec_soft_cosine_similarity(ip_text1, extracted_text)
                if(sim>0.53):
                    prereq_waived = True
            elif(selected_prerequisite == "8"):
                sim = get_word2vec_soft_cosine_similarity(ip_text2, extracted_text)
                if(sim>0.53):
                    prereq_waived = True
            elif(selected_prerequisite == "7"):
                sim = get_word2vec_soft_cosine_similarity(ip_text3, extracted_text)
                if(sim>0.57):
                    prereq_waived = True
            else:
                print("Error occured, please try again.")

            def delete_row(student_id, selected_prerequisite):
                    ...
                    with connection.cursor() as cursor:
                        ...
                        cursor.execute(query, (student_id, selected_prerequisite))
                        connection.commit()

            print(prereq_waived)
            if (prereq_waived):
                delete_row(student_id, selected_prerequisite)
                return "True"               
            else:
                return "False"
