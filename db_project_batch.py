...
path = 'C:/Users/VARAD/Documents/DB_Project_Get_Model_Accuracy/'
...
def get_tf_idf_cosine_similarity(text1, text2):
    text1 = text1.lower()
    text2 = text2.lower()
...
def get_word2vec_cosine_similarity(text1,text2):
    ...
    cos_sim = 1 - spatial.distance.cosine(get_vector(lemmatized_output1), get_vector(lemmatized_output2))
    return cos_sim
...
def getSubjectAccuracy(path,ip_text,thresholds,subject):
    file_no = 0
    global correct_list,total_files,correct_op_tf_idf,correct_word_cosine,correct_word_soft_cosine
    for filename in glob.glob(os.path.join(path, '*.pdf')):
        reader = PdfReader(os.path.join(os.getcwd(), filename))
        text = ""
        for page in reader.pages:
            text += page.extract_text() + " "
        
        sim1 = get_word2vec_cosine_similarity(ip_text, text)
        sim2 = get_word2vec_soft_cosine_similarity(ip_text, text)
        sim3 = get_tf_idf_cosine_similarity(ip_text, text)
...
getSubjectAccuracy(path+'DiscreteMaths',ip_text1,(0.91,0.53,0.43),'DiscreteMaths')
getSubjectAccuracy(path+'DataStructures',ip_text2,(0.84,0.53,0.29),'DataStructures')
getSubjectAccuracy(path+'OperatingSystems',ip_text3,(0.92,0.57,0.31),'OperatingSystems')

if(total_files==0):
    print("Error: The dataset folders seem to be incorrectly named or empty. They should be named DiscreteMaths, DataStructures and OperatingSystems.")
else:
